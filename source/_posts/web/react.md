...
虽然你可以添加一个ref到子组件，但这并不是一个理想的解决方案，因为你只会获取到组件实例而非DOM节点，而且这样也无法用于函数类型组件。因此，这里推荐在子组件内暴露一个特殊的`prop`，使子组件能够通过该`prop`接收到一个任意名称的函数（*例如下面函数中的inputRef*），然后通过`ref`属性将该函数关联到DOM节点，最终使得父组件能够通过一个中间层级组件传递其`ref`回调函数至DOM节点，并且这种方式能够同时应用在类组件和函数组件当中，示例代码如下：
